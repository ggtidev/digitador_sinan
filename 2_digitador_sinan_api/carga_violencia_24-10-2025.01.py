...
with open(log_path, "w", encoding="utf-8") as log_file:
    ...
    for record, dados in dados_por_record.items():
        ...
        if un_val is not None:
            ...
            # --- REGRA 2.1 ---
            # Se for diferente de 1, força o valor a '7' (padronização SINAN)
            if un_val == "1":
                un_val_final = "1"
            else:
            ...
            # --- REGRA 2.2 ---
            # Define a origem do nome da unidade conforme o código
            if un_val_final in ["2", "3", "4", "6", "7"]:
                nm_un_vio = obter_valor(campos_presentes, "nm_un_vio")
                origem_nome = "nm_un_vio"
            elif un_val_final in ["1", "5"]:
                nm_un_vio = obter_valor(campos_presentes, "us_vio", "nome_unidade_saude")
                origem_nome = "us_vio"
            else:
                nm_un_vio = None
                origem_nome = "nenhuma"

            print(f"[DEBUG] Record {record} → Origem do nome da unidade: {origem_nome} | Valor: '{nm_un_vio}'")

            # Se ainda não tiver nome, marca como faltante
            if not nm_un_vio:
                campos_faltando.append(f"{origem_nome} (não informado para un_not_vio = {un_val_final})")

        else:
            # Caso o campo 'un_not_vio' esteja ausente
